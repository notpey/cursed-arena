...
function kindGlyph(kind: BattleEventKind): { icon: string; tone: string } {
  switch (kind) {
    case 'damage': return { icon: 'DMG', tone: 'text-ca-red bg-ca-red/15 border-ca-red/30' }
    case 'heal': return { icon: 'HP', tone: 'text-emerald-300 bg-emerald-400/12 border-emerald-400/30' }
    case 'status': return { icon: 'ST', tone: 'text-amber-300 bg-amber-300/12 border-amber-300/30' }
    case 'defeat': return { icon: 'KO', tone: 'text-ca-red bg-ca-red/20 border-ca-red/35' }
    case 'victory': return { icon: 'WIN', tone: 'text-amber-300 bg-amber-300/18 border-amber-300/35' }
    case 'phase': return { icon: 'RD', tone: 'text-sky-300 bg-sky-400/10 border-sky-400/30' }
    case 'action': return { icon: 'ACT', tone: 'text-ca-teal bg-ca-teal/12 border-ca-teal/30' }
    case 'system':
    default: return { icon: 'SYS', tone: 'text-ca-text-3 bg-white/6 border-white/12' }
  }
}

function logLabel(event: BattleEvent) {
  ...
  if (message.includes('canceled') || message.includes('interrupted')) return 'INTERRUPTED'
  if (message.includes('cursed energy') || message.includes('energy refreshed')) return 'RESOURCE'
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      <span
                        className={cn(
                          'inline-grid h-[0.95rem] min-w-[1.4rem] place-items-center rounded-[0.18rem] border px-1 text-[0.45rem] leading-none',
                          glyph.tone,
                        )}
                        aria-hidden
                      >
